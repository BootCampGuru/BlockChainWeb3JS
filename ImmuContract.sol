 ...
 function getProviderByAddress(address host) view public returns(string, bool, address, string, string, uint, uint)
{
 ...
 function getUserByAddress(address host) view public returns(string, string, address, string, uint, uint)
 {
     return (UserList[host].firstName, UserList[host].lastName,UserList[host].provider, UserList[host].certificateHash, host.balance, UserList[host].timestamp );
...
function AddProviderInfo(address host, string name) public returns(bool)
{
  ...
  //ProviderList[msg.sender].providerHash = "QmSP8LtdoWzcZv9vo2Ee6RDxeXFSNpenVDWVd1MhQAopGW";
  clearedTransactions[msg.sender]  = 0;
...
// }
 ...
 function RegisterUser(string firstname, string lastname, address provider, uint amount) public returns(bool) {
   ...
   UserList[msg.sender].timestamp = block.timestamp;
   //owner.send(amount);
